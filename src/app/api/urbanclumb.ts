...
export async function LastRouteSet(locationId: string): Promise<LastSetResponse> {
    const response = await fetch(`https://api-prod.urbanclimb.com.au/widgets/last-set?branch=${locationId}`);
    if (!response.ok) {
        throw new Error('Network response was not ok');
    }
    return response.json();
